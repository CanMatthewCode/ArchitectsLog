...
def phase_duration_by_project_with_name(project_id: int, cur: sqlite3.Cursor,
	start_date: Optional[int] = None,
	end_date: Optional[int] = None) -> list[tuple[int, int, str]]:
	"""Retrieve the summation of all of a project's per phase duration,
	returns a list of tuples containing (phase_id, sum duration_minutes, project_name) 
	per phase"""
	if start_date and end_date:
		sql = "SELECT phase_id, ROUND(SUM(duration_minutes)/60.0, 2), \
			projects.project_name \
			FROM time_entries\
			INNER JOIN projects ON time_entries.project_id = projects.project_id\
			WHERE time_entries.project_id = ? and start_time >= ? and start_time <= ? \
			GROUP BY phase_id \
			ORDER BY phase_id ASC"
		cur.execute(sql, (project_id, start_date, end_date))
	else:
...
def total_number_of_projects_by_phase(cur:sqlite3.Cursor, start_date: Optional[int] = None,
	end_date: Optional[int] = None) -> list[tuple[int, int]]:
	"""Retrieve the total number of project_ids per phase in the time_entries table"""
	if start_date and end_date:
		sql = "SELECT phase_id, COUNT(DISTINCT project_id) FROM time_entries\
			WHERE start_time >= ? AND start_time <= ? \
			GROUP BY phase_id ORDER BY phase_id ASC"
		cur.execute(sql, (start_date, end_date))
	else:
...
def project_start_date(project_id: int, cur:sqlite3.Cursor) -> int:
	...
	sql = "SELECT start_date FROM projects WHERE project_id = ?"
	cur.execute(sql, (project_id,))
	return cur.fetchone()
