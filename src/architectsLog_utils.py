...
def program_loadup() -> str:
	"""Function to run demo database when program first loads, then user database
	once created"""
	...
	if not settings_path.exists():
		...
		# For packaged program
		if getattr(sys, "frozen", False):
			basedir = sys._MEIPASS
		# For command line run program
		else:
...
def new_database() -> None:
	"""Function to create new, empty databasae for user, change settings to 
...
def load_database() -> None:
	...
	and to archive currently loaded database"""
	dirs = PlatformDirs("ArchitectsLog", ensure_exists=True)
	data_dir = dirs.user_data_path
